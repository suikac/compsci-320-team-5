import { ArgumentsHost, CanActivate, Catch, ExceptionFilter, ExecutionContext, HttpCode, HttpException, HttpStatus, Inject, Injectable, UseFilters } from "@nestjs/common";
...
export class JwtGuard implements CanActivate {
  constructor(
    @Inject("LOGIN_SERVICE") private loginService: ClientProxy
  ) {}
  ...
  async canActivate(context: ExecutionContext) {
    ...
    try {
      let response: any = await firstValueFrom(this.loginService.send(cmd, {token: tokenString}))
